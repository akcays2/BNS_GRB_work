...
%
%
We can see from Table~\ref{table:rates} that an event as near as GW170817 is a one-per-decade occurence,
...
In this case, early warnings would be sent by ET at least once a week and as often as 3 times per day.
It is not feasible or reasonable to expect a system to be in place to observe each one of these events
so it will make sense to be selective and focus on the best candidates which will be the nearby sources.
And as we write below, ET's limitations with localisation will act as a selection filter.
...
{\bf SA to do:} make ETC thick red, make ETB and ALIGO dashed, bigger x-y labels and tick marks, bigger legend cutoff plot at ~2000Hz?
}
\label{fig:ETB2030}
\end{figure}
