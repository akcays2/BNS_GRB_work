...
This third detector will most likely be the KAGRA's future version dubbed ``+''.
\sa{TO DO: add KAGRA3+, 6e-22 at 6 Hz or is it 1e-21?}


In short within 400\,Mpc, each year, we can expect 5 BNS sources to be localised to LSST's viewfield 1.5 hours
before the merger. We can expect an additional $\sim 15$ more BNS transients with initial $\Delta\Omega \approx 20\,$deg$^2$ to be localised to fall within LSST's viewfield about one hour before the merger.

We envision a stepped localisation procedure whereby ET conducts the operations alone until $f\sim 5\,$Hz at which
point CE joins in and finally around $f\sim 6\,$Hz KAGRA3+ joins the network ...
