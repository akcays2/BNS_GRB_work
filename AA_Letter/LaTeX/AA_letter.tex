...
As the true ISCO frequency is likely larger than $f_\text{ISCO}$ \citep{Marronetti:2003hx}, the inspirals would continue to nearly 2\,kHz indicated by the faded lines in the plot. %(drawn to 5\,kHz for aesthetic reasons).
RMS averaged BNS strains in an L-topology (like CE's) are 2/3 of strains in $\triangle$-topology (ET's) \citep{Akcay18}.
Thus, we rescale the CE curve by 3/2 to account for this (to be able to include it in this plot).
\sa{TO DO  bigger x-y labels and tick marks, bigger legend, include LIGO2017 for comparison?}
}
\label{fig:ETB2030}
\end{figure}
%
...
Given that its sensitivity increases steeply between 5 and 10\,Hz, CE accumulates $\text{SNR} = 5$ with 1.5\,hours left to merger and SNR = 15 with 1.25\,hours left.
Using $\rho_\text{tot} = \left(\rho_\text{ET}^2+\rho_\text{CE}^2\right)^{1/2}$, we have a total network
SNR of $\{18.8,27.4,37.9\}$ for $\tau_\text{insp}=\{1.5, 1.25, 1\}\,$hour, respectively.
As localisation improves with increasing SNR, this means that an initial sky region of $\sim 100\,$deg$^2$
can be narrowed as the BNS inspirals through its second to last hour before the merger.

This region can be further decreased once the BNS enters a third detector's frequency band.
This third detector will most likely be some future version of KAGRA. 
We are not aware of any strain data for a third generation KAGRA. 
However, there are plans to update it to KAGRA+ in mid 2020s with increased strain sensitivity which we show as the light brown curve in Fig.~\ref{fig:ETB2030}
Using the same analysis as for CE, we obtain that KAGRA+ will pick up the 400-Mpc inspiral at $\sim 10\,$Hz,
but will accumulate $\text{SNR}=5$ within a minute. 
For a less likely 100-Mpc source, KAGRA+ would accumulate $\text{SNR} > 15$ more than a half hour before the merger.
Thus, for nearby sources even mid 2020s KAGRA+ could contribute to localisation. \sa{How much time does LSST need before moving to final target?}
Once again, this is rather a pessimistic estimation as we expect that KAGRA's 3G successor 
will be more sensitive than KAGRA+.
...
We envision a three-stage localisation procedure whereby ET conducts the operations alone until $f\lesssim 5\,$Hz 
--- roughly two hours before the merger --- at which point CE joins in and finally around $f\lesssim 10\,$Hz KAGRA+ (or 3G) starts contributing to the localisation endeavour with $\gtrsim 15\,$ minutes left to merger.
For sources closer than 400\,Mpc these estimations will improve.
