...
\label{sect:et}
In this section, we want to compute advanced warning times ($T_\text{AW}$) ET will provide.
...
%
\be
\rho_{\text{ET}} = \f{6}{5}A\, h_0  (1+z)^{-1/6} \left[\int_{f_0}^{f_\text{ISCO}} d f\, \f{f^{-7/3}}{S_n(f)}\right]^{1/2} \label{eq:ET_SNR},
\ee
%
where %$z$ is the source redshift and
$f_\text{ISCO}$ is the frequency at which the inspiral transitions to a mutual plunge. Here, we use the standard approximation $f_\text{ISCO} \approx \tfrac{c^3}{6^{3/2}\pi G (m_1+m_2)} \simeq 1571 \left(\f{2.8M_\odot}{m_1+m_2}\right)\text{Hz}$.
The factor of $6/5$ in Eq.~(\ref{eq:ET_SNR})
is due to a RMS-averaging over the angles $\{\theta,\phi,\iota,\psi\}$. 
