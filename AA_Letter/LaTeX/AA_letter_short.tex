...
\abstract{
The discovery of gravitational waves from merging compact objects has opened up a new window on the Universe. Planned third generation GW detectors such as the Einstein Telescope will potentially deliver hundreds of such events at redshifts below $z\sim0.1$. Finding electromagnetic counterparts to these events will be a major observational challenge. We demonstrate how the Einstein Telescope will provide advance warning of 
...
There has been a considerable amount of discussion in the literature as to the optimal strategy to identify an EM counterpart to future GW transients \citep{2016ApJ...820..136G, 2011MNRAS.415L..26C, 2016A&A...592A..82G, 2017ApJ...834...84C, 2014MNRAS.437..649S, 2016MNRAS.462.1085A}. In most cases however, a large number of candidates will be found within the search region, for which further spectroscopic followup observations will be required. This spectroscopic classification bottleneck will remain a problem into the foreseeable future.
...
present a brief summary of these results and their implications for followup observations of electromagnetic counterparts.
%We
...
A passing GW induces a scalar response in a given interferometer (IFO) known as the GW strain which is a function of both GW polarization the IFO antenna pattern. 
...
We chose a conservatve threshold SNR of 15. % is not set in stone;
...
In traditional amplitude spectral density plots, inspiral GW strains scale as $ \sqrt{f} f^{-7/6} = f^{-2/3}$ \citep{Colpi_Sesana},
...
in units of \mf{Is this really in units of D$^{-3}$?}$D^{-3}\,\text{yr}^{-1}$ within each concentric shell.}
...
We base our event rate calculations on $R=1540^{+3200}_{-1220}\,\text{Gpc}^{-3}\,\text{yr}^{-1}$ inferred from GW170817 \citep{GW170817}. Assuming flat spatial geometry, this translates roughly to $330^{+690}_{-260}$ BNS events per year within 600\,Mpc. We partition this volume into concentric shells with at integer multiples of 100 Mpc. For each shell, we show the event rates in Table~\ref{table:ET} along with the corrsponding redshift assuming
...
Once again, this is a rather pessimistic estimation as we expect KAGRA's third generation successor 
...
We ran Monte Carlo simulations to obtain an estimate of the number of unrelated transients that may be found in a search for an EM counterpart to a GW. We took the volumetric Type Ia SN rate from \cite{2010ApJ...713.1026D}, and assumed that the GW event could be localised to a region of $\sim$30 deg$^2$. We further assumed an optical survey with a cadence of 4 days and a limiting magnitude $m_r\sim$22, and that the kilonova had an absolute magnitude of $M_r=-15$ at peak. We then calculated the number of Type Ia SNe that would be detected by the survey with a magnitude comparable ($\pm1$mag) to the kilonova, {\it and} where the SN would have not been detected on the previous image taken of the field four nights earlier. The results of this are shown in Fig. \ref{fig:SNIa}, where we find that for a GW at a distance of a few hundred Mpc, we will typically have four 
...
\caption{The probability of finding a given number of SNe Ia (that are likely to be confused with a kilonova), as a function of distance. We assume a survey with a four night cadence reaching $m_r\lesssim$22, a GW localised to 30 deg$^2$, and a kilonova  $M_r=-15$.}
\label{fig:SNIa}
\end{figure}
